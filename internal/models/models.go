...
func (m *DBModel) Authenticate(email, password string) (int, error) {
	...
	var id int
	var hashedPaswword string

	row := m.DB.QueryRowContext(ctx, "select id, password from users where email = ?", email)
	err := row.Scan(&id, &hashedPaswword)
	if err != nil {
		return id, err
	}

	err = bcrypt.CompareHashAndPassword([]byte(hashedPaswword), []byte(password))
...
// GetAllOrdersPaginated returns a slice of a subset of orders
func (m *DBModel) GetAllSubsrciptionsPaginated(pageSize, page int) ([]*Order, int, int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
